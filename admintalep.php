...
if($islem == 'kapat'){
	...
	if($admin_mi != '1'){
		alertmsg("","Bu kısım adminlere özel dostum maalesef :)","index.php","warning");
		return;
	}
	
$ticket_durum = '<input type="button" class="btn btn-success" style="background-color:#b6a8a5;" value="Kapalı">';

$devamke2 = $sql->prepare("UPDATE ticket SET durum=? WHERE id=?");
$devamke2->execute(array($ticket_durum,$kapatid));

if($devamke2){
	alertmsg("","Destek Talebi Kapatıldı.","$geridon","success");
	return;
}else{
	alertmsg("","Teknir Bir Problem Oluştu.","$geridon","warning");
	return;
}
...
$ticket_durum = '<input type="button" class="btn btn-success" style="background-color:#b6a8a5;" value="Yanıtlandı">';

$devamke = $sql->prepare("INSERT INTO ticketre SET cevap=?,cevapsahibi=?,tarih=?,ticketid=?");
$devamke->execute(array($cvcevap,"Yetkili","$tarih - $saat",$ticketi));

$devamke2 = $sql->prepare("UPDATE ticket SET durum=? WHERE id=?");
$devamke2->execute(array($ticket_durum,$ticketi));
...
?>
											 <tr>
												...
												<td><? echo $dttarih; ?></td>
												<td><? echo $dtdurum; ?></td>
												<td><div class="card-header"><a href="admintalep.php?id=<?echo $_GET['id'];?>&talep=kapat" class="btn btn-success">
												Talebi Kapat
												</a></td>
											</tr>
											</tbody>
